...
export async function updateGameProgress(firstName, lastName, department, progressData) {
  try {
    ...
    const gameInProgress = await kv.get(`game:${playerId}`)
    if (!gameInProgress) {
      throw new Error('No game in progress found')
    }

    // ✅ COMPREHENSIVE STATE UPDATE - Merge all game state data
    const updatedGameData = {
      ...
      gameState: {
        ...
        // ✅ CRITICAL: Update questions and puzzle data for exact restoration
        questionsForLevels:
          progressData.questionsForLevels || gameInProgress.gameState?.questionsForLevels || {},
        finalPuzzle: progressData.finalPuzzle || gameInProgress.gameState?.finalPuzzle || null,
        currentSolution:
