...
console.log('🔍 Environment check:', {
  ...
  KV_REST_API_URL: process.env.REDIS_KV_REST_API_URL ? 'SET' : 'MISSING',
  KV_REST_API_TOKEN: process.env.REDIS_KV_REST_API_TOKEN ? 'SET' : 'MISSING',
  REDIS_URL: process.env.REDIS_REDIS_URL ? 'SET' : 'MISSING',
})

console.log('🔍 Using PRODUCTION Redis for both dev and prod:', {
  url: process.env.REDIS_KV_REST_API_URL
    ? process.env.REDIS_KV_REST_API_URL.substring(0, 30) + '...'
    : 'MISSING',
  token: process.env.REDIS_KV_REST_API_TOKEN ? 'SET' : 'MISSING',
})
...
export async function checkPlayerParticipation(firstName, lastName, department) {
  try {
    ...
    // Check if player already completed the tournament
    ...
    // ✅ ANTI-CHEAT: Also check for games in progress
    const gameInProgress = await kv.get(`game:${playerId}`)
    if (gameInProgress) {
      return {
        hasParticipated: true,
        gameInProgress: true,
        message: 'You already have a game in progress. Please continue your current game.',
...
// ✅ ANTI-CHEAT: Track when a player starts the game
export async function startPlayerGame(firstName, lastName, department, gameStartTime = null) {
  try {
    const playerId = generatePlayerId(firstName, lastName, department)

    // Check if player already has completed the tournament
    const existingPlayer = await kv.get(`player:${playerId}`)
    if (existingPlayer) {
      return {
        success: false,
        message: 'You have already completed this tournament.',
    ...
    // Mark game as started with comprehensive game state
    const gameStartData = {
      playerId,
      firstName,
      lastName,
      department,
      startedAt: getCopenhagenTime(),
      gameStartTime: gameStartTime || Date.now(), // When the actual game timer started
      timestamp: Date.now(),
      status: 'in-progress',
      currentRoomIndex: 0,

      // ✅ ANTI-CHEAT: Complete game state tracking
      gameState: {
        // Player scoring state (prevents score manipulation)
        wrongAnswerPenalties: 0,
        hintsUsed: 0,
        completionBonus: 0,

        // Room progress state
        currentRoomId: 'it-security',
        levelStatus: {
          level1: 'unlocked',
          level2: 'locked',
          level3: 'locked',
        ...
        // ✅ CRITICAL: Initialize questions and puzzle data (will be set when room is set up)
        questionsForLevels: {},
        finalPuzzle: null,
        currentSolution: '',
        currentHints: {
        ...
        // Puzzle progress state
        puzzleStates: {},

        // Session integrity
        sessionId: `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        lastUpdated: getCopenhagenTime(),
      },
    }

    await kv.set(`game:${playerId}`, gameStartData)

    // Set expiration for game data (24 hours) in case player abandons
    await kv.expire(`game:${playerId}`, 86400) // 24 hours
...
// ✅ ANTI-CHEAT: Update comprehensive game progress (prevents all cheating)
export async function updateGameProgress(firstName, lastName, department, progressData) {
  try {
    const playerId = generatePlayerId(firstName, lastName, department)

    console.log('🔍 Looking for game in progress for player:', playerId)

    // Get existing game data
    const gameInProgress = await kv.get(`game:${playerId}`)
    if (!gameInProgress) {
      console.log('⚠️ No game in progress found - creating new game record')

      // Create a new game record if it doesn't exist (fallback for race conditions)
      const newGameData = {
        playerId,
        firstName,
        lastName,
        ...
        timestamp: Date.now(),
        currentTime: progressData.currentTime || gameInProgress.currentTime || Date.now(),
        status: 'in-progress',
        currentRoomIndex: 0,
        gameState: {
          wrongAnswerPenalties: 0,
          ...
          completionBonus: 0,
          currentRoomId: 'it-security',
          levelStatus: { level1: 'unlocked', level2: 'locked', level3: 'locked' },
          collectedHints: [],
          currentQuestionIndex: { level1: 0, level2: 0, level3: 0 },
          correctAnswersCount: { level1: 0, level2: 0, level3: 0 },
          questionsForLevels: {},
          finalPuzzle: null,
          currentSolution: '',
          currentHints: { level1: null, level2: null, level3: null },
          sessionId: `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
          lastUpdated: getCopenhagenTime(),
        },
      }

      await kv.set(`game:${playerId}`, newGameData)
      await kv.expire(`game:${playerId}`, 86400) // 24 hours

      console.log('✅ Created new game record for player')
      return { success: true, gameData: newGameData }
    }

    console.log('🔍 Updating game data for player:', playerId)
    console.log('📊 Progress data received:', Object.keys(progressData))

    // ✅ COMPREHENSIVE STATE UPDATE - Merge all game state data with error handling
    const updatedGameData = {
      ...gameInProgress,
      currentRoomIndex: progressData.currentRoomIndex || gameInProgress.currentRoomIndex,
      lastUpdateAt: getCopenhagenTime(),
      lastUpdateTimestamp: Date.now(),

      // Preserve the original game start time (anti-cheat)
      gameStartTime: gameInProgress.gameStartTime,
      currentTime: progressData.currentTime || gameInProgress.currentTime || Date.now(),
      // ✅ ANTI-CHEAT: Update comprehensive game state
      gameState: {
        ...gameInProgress.gameState,

        // Update player scoring state (prevents score manipulation)
        wrongAnswerPenalties:
          progressData.wrongAnswerPenalties ?? gameInProgress.gameState?.wrongAnswerPenalties ?? 0,
        hintsUsed: progressData.hintsUsed ?? gameInProgress.gameState?.hintsUsed ?? 0,
        completionBonus:
          progressData.completionBonus ?? gameInProgress.gameState?.completionBonus ?? 0,

        // Update room progress state
        currentRoomId:
          progressData.currentRoomId || gameInProgress.gameState?.currentRoomId || 'it-security',
        levelStatus: progressData.levelStatus ||
          gameInProgress.gameState?.levelStatus || {
            level1: 'unlocked',
            level2: 'locked',
            level3: 'locked',
          },
        collectedHints:
          progressData.collectedHints || gameInProgress.gameState?.collectedHints || [],
        currentQuestionIndex: progressData.currentQuestionIndex ||
          gameInProgress.gameState?.currentQuestionIndex || {
            level1: 0,
            level2: 0,
            level3: 0,
          },
        correctAnswersCount: progressData.correctAnswersCount ||
          gameInProgress.gameState?.correctAnswersCount || {
            level1: 0,
            level2: 0,
            level3: 0,
          },

        // ✅ CRITICAL: Safely handle complex question data
        questionsForLevels: (() => {
          try {
            return (
              progressData.questionsForLevels || gameInProgress.gameState?.questionsForLevels || {}
            )
          } catch (e) {
            console.warn('⚠️ questionsForLevels data issue, using fallback:', e.message)
            return {}
          }
        })(),
        ...
        finalPuzzle: (() => {
          try {
            return progressData.finalPuzzle || gameInProgress.gameState?.finalPuzzle || null
          } catch (e) {
            console.warn('⚠️ finalPuzzle data issue, using fallback:', e.message)
            return null
          }
        })(),

        currentSolution:
          progressData.currentSolution || gameInProgress.gameState?.currentSolution || '',
        currentHints: progressData.currentHints ||
          gameInProgress.gameState?.currentHints || {
            level1: null,
            level2: null,
            level3: null,
          },

        // Update puzzle progress state
        puzzleStates: progressData.puzzleStates || gameInProgress.gameState?.puzzleStates || {},

        // Update session integrity
        sessionId:
          gameInProgress.gameState?.sessionId ||
          `${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
        lastUpdated: getCopenhagenTime(),
      },
    }

    console.log('💾 Saving updated game data to database...')
    await kv.set(`game:${playerId}`, updatedGameData)

    // Refresh expiration (24 hours)
    await kv.expire(`game:${playerId}`, 86400)

    console.log('✅ Game data successfully updated for player:', playerId)
...
// ✅ ANTI-CHEAT: Complete/clear the game in progress
export async function completePlayerGame(firstName, lastName, department) {
  try {
    const playerId = generatePlayerId(firstName, lastName, department)

    // Remove the game in progress marker
    await kv.del(`game:${playerId}`)
...
export async function savePlayerScore(playerData) {
  try {
    ...
    const newPlayerData = {
      ...
      completionTime: playerData.completionTime || getCopenhagenTime(),
      submittedAt: getCopenhagenTime(),
      timestamp: Date.now(),
    }
    ...
    // ✅ ANTI-CHEAT: Clear the game in progress marker
    await completePlayerGame(playerData.firstName, playerData.lastName, playerData.department)
  ...
  try {
    ...
    if (!usingSortedSet || !topPlayerKeys || !topPlayerKeys.length) {
      ...
      if (directPlayers.length > 0) {
        ...
        directPlayers.sort((a, b) => {
          if (b.score !== a.score) return b.score - a.score
          // Tie-break: sort by timeSpent ascending (fastest first)
          // Convert timeSpent (string) to seconds for comparison
          const parseTime = (t) => {
