...
function generatePlayerId(firstName, lastName, department) {
  return `${firstName.toLowerCase().trim()}-${lastName.toLowerCase().trim()}-${department.toLowerCase().replace(/\s+/g, '-')}`
}
...
export async function savePlayerScore(playerData) {
  try {
    ...
    // ✅ DUPLICATE PREVENTION - Only save if better score or first attempt
    if (existingPlayer) {
      if (playerData.score <= existingPlayer.score) {
        return { 
          success: false, 
          message: 'Score not improved. Previous best score maintained.',
      ...
      await kv.zrem('leaderboard', `player:${playerId}`)
    }
...
export async function getLeaderboard(limit = 50) {
  try {
    ...
    const playersData = await Promise.all(
      topPlayerKeys.map(key => kv.get(key))
    )
...
export async function getAllPlayersForAdmin() {
  try {
    ...
    const playersData = await Promise.all(
      allPlayerKeys.map(key => kv.get(key))
    )
...
async function getTournamentStats() {
  try {
    return await kv.get('tournament:stats') || {
      totalPlayers: 0,
      averageScore: 0,
      highestScore: 0,
      lowestScore: 100
