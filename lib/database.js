...
export async function getLeaderboard(limit = 50) {
  try {
    ...
    console.log('🔍 Fetching top players from leaderboard...')
    const topPlayerKeys = await kv.zrevrange('leaderboard', 0, limit - 1)
    console.log(`🔍 Found ${topPlayerKeys?.length || 0} player keys:`, topPlayerKeys)

    if (!topPlayerKeys || !topPlayerKeys.length) {
      console.log('🔍 No players found, returning empty leaderboard')
      return {
        leaderboard: [],
        totalPlayers: 0,
        averageScore: 0,
    ...
    console.log('🔍 Fetching full player data...')
