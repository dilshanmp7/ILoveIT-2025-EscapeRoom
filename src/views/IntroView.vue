...
<template>
  ...
  >
    ...
    <div class="w-full h-full flex items-center justify-center text-white p-2 sm:p-4 relative">
      <div
        class="flex flex-col items-center justify-center max-w-sm mobile:max-w-md sm:max-w-lg laptop:max-w-2xl large:max-w-4xl w-full py-4"
      >
        <img
          :src="dhlLogo"
          alt="DHL Logo"
          class="w-16 sm:w-24 laptop:w-20 large:w-32 mb-2 sm:mb-3 laptop:mb-2"
        />

        <h1
          class="text-lg mobile:text-xl sm:text-2xl laptop:text-2xl large:text-4xl font-bold font-delivery text-dhl-yellow mb-2 sm:mb-4 laptop:mb-3 text-center drop-shadow-lg"
        >
          The IT Lockdown
        </h1>

        <div
          class="flex items-center justify-center gap-2 sm:gap-3 laptop:gap-4 large:gap-5 mb-3 sm:mb-4 laptop:mb-4"
        >
        ...
        <div v-if="hasAlreadyPlayed && finalResult" class="text-center w-full px-2 sm:px-4">
          <div
            class="bg-white p-3 sm:p-6 laptop:p-8 rounded-lg shadow-2xl max-w-xs sm:max-w-md laptop:max-w-lg mx-auto"
          >
            <div class="mb-3 sm:mb-4">
              <div
                class="w-12 h-12 sm:w-16 sm:h-16 laptop:w-20 laptop:h-20 bg-dhl-red rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4"
              >
                <svg
                  class="w-6 h-6 sm:w-8 sm:h-8 laptop:w-10 laptop:h-10 text-white"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
              ...
              <h2 class="text-lg sm:text-xl laptop:text-2xl font-bold text-gray-800 mb-2">
                Tournament Complete!
              </h2>
              <p class="text-sm sm:text-base laptop:text-lg text-gray-600 mb-3 sm:mb-4">
                {{ finalResult.name }}
              </p>
            </div>
            <div class="space-y-2 sm:space-y-3">
              <div class="flex justify-between items-center py-2 border-b border-gray-200">
                <span class="text-xs sm:text-sm laptop:text-base text-gray-600">Final Score:</span>
                <span class="text-lg sm:text-2xl laptop:text-3xl font-bold text-dhl-red">{{
                  finalResult.score
                }}</span>
              </div>
              <div class="flex justify-between items-center py-2 border-b border-gray-200">
                <span class="text-xs sm:text-sm laptop:text-base text-gray-600">Time Taken:</span>
                <span class="text-sm sm:text-base laptop:text-lg font-semibold">{{
                  finalResult.timeSpent
                }}</span>
              </div>
              <div v-if="finalResult.rank" class="flex justify-between items-center py-2">
                <span class="text-xs sm:text-sm laptop:text-base text-gray-600"
                  >Tournament Rank:</span
                >
                <span
                  class="text-sm sm:text-base laptop:text-lg font-bold text-dhl-yellow bg-gray-800 px-2 sm:px-3 py-1 rounded"
                  >#{{ finalResult.rank }}</span
                >
              </div>
            </div>
            <div class="mt-4 sm:mt-6 p-3 sm:p-4 bg-yellow-50 border-l-4 border-dhl-yellow rounded">
              <p class="text-xs sm:text-sm laptop:text-base text-gray-700">
                <strong>🏆 Tournament Rules:</strong> Each player can participate only once to
                ensure fair competition for all DHL employees.
              </p>
            </div>
          </div>
          <div class="mt-4">
            <button
              ...
              class="bg-dhl-yellow text-black px-4 sm:px-6 py-2 sm:py-3 rounded-lg text-sm sm:text-base font-bold hover:bg-yellow-400 transition-colors touch-manipulation"
            >
        ...
        <div v-else class="w-full flex flex-col items-center">
          <p
            class="text-xs sm:text-base laptop:text-sm max-w-3xl text-center mb-3 sm:mb-4 px-2 sm:px-4"
          >
            Enter your details to begin your mission. You have 45 minutes to escape the IT lockdown.
            Good luck!
          </p>
          <div
            class="w-full max-w-xs sm:max-w-md laptop:max-w-lg bg-white/50 p-3 sm:p-4 rounded-lg shadow-lg border border-white/20 mx-2 sm:mx-4"
          >
            <div class="grid grid-cols-1 gap-2 laptop:gap-3">
              <div>
                <label
                  for="firstName"
                  class="block text-xs sm:text-sm font-bold text-gray-800 mb-0.5"
                  >First Name</label
                >
                <input
                  v-model="firstName"
                  id="firstName"
                  type="text"
                  class="w-full p-1.5 sm:p-2 border border-gray-400 rounded-md text-gray-900 focus:ring-dhl-red focus:border-dhl-red text-sm"
            ...
            <div
              v-if="errorMessage"
              class="mt-3 p-2 bg-red-100 border border-red-400 text-red-700 rounded text-xs sm:text-sm"
            >
              {{ errorMessage }}
            </div>
            <button
              ...
              class="w-full bg-dhl-red text-white font-bold py-2 sm:py-3 rounded-lg text-sm mt-3 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed hover:bg-red-700 touch-manipulation flex items-center justify-center min-h-[40px]"
            >
              <svg
              ...
              {{ isCheckingPlayer ? 'Checking...' : 'Begin Mission' }}
            </button>
